...
def trace_spans(base: str, trace_id: str) -> list[dict]:
    ...
    MLflow's REST surface for spans is not stable across releases: 3.14.0 serves
    no ``/traces/{id}/spans`` route at all, and a 404 there is a version
    difference rather than a missing trace. The Python client abstracts over it,
    so it is preferred when importable; the REST attempts are the fallback for a
    host that has only the standard library.

    Returns an empty list rather than raising, so the trace-level summary — which
    REST *does* serve everywhere, and which carries the readable question and
    answer this pipeline exists to produce — is still printed.
    """

    try:
        import mlflow  # noqa: PLC0415 - optional, and only on this path

        mlflow.set_tracking_uri(base)
        trace = mlflow.get_trace(trace_id)
        spans = getattr(trace, "data", None)
        spans = getattr(spans, "spans", None) or []
        return [span.to_dict() if hasattr(span, "to_dict") else dict(span) for span in spans]
    except ImportError:
        pass
    except Exception as error:  # noqa: BLE001 - fall through to REST
        print(f"  (mlflow client could not read spans: {error})")

    for path in (
        f"/api/3.0/mlflow/traces/{trace_id}/spans",
        f"/ajax-api/3.0/mlflow/traces/{trace_id}/spans",
    ):
        try:
            return api(base, path, method="GET").get("spans", [])
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Defaults to MLFLOW_TRACKING_URI, then http://localhost:5000. "
        "Set MLFLOW_PORT and this must move with it.",
    )
    ...
    for trace in traces:
        ...
        # The readable question and answer the telemetry pipeline puts on the
        # root span. Printed from the trace summary so they show even when the
        # span tree is unavailable on this MLflow release.
        for label, key in (("question", "request_preview"), ("answer  ", "response_preview")):
            value = trace.get(key)
            if value:
                print(f"  {label}: {short(value, 160)}")
        if not spans:
            print("  (no span tree available from this MLflow release over REST; "
                  "install the mlflow client for the full hierarchy)")
