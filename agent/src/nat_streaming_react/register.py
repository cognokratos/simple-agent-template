...
This function sees the incoming ``ChatRequest`` before anything else does, so
it is where the readable *question* is captured for the telemetry pipeline
(see ``nat_streaming_react.observability.trace_content``). It is **not** the
right place to capture the *answer*: the guardrail middleware wraps this
function and runs strictly after it, so the text produced here is pre-rail —
recording it as "the answer" would let a masked or blocked response leak into
the readable trace. The answer is instead captured where the guardrail
middleware actually releases text downstream
(``nat_streaming_react.text_guardrails.TextGuardrailsMiddleware``), which is
the true output boundary for both streaming and non-streaming responses.

NAT owns the canonical request trace; Guardrails joins it through the trace
context established at the HTTP boundary by
``nat_streaming_react.observability.trace_context``.
"""
...
# Imported for their NAT component registration side effects.
from nat_streaming_react.otel_setup import configure_opentelemetry
from nat_streaming_react.llm_config import (
    openai_optional_params_langchain as _openai_optional_params_langchain,
)
from nat_streaming_react.llm_config import (
...
async def streaming_react_agent_workflow(
    ...
    async def _response_fn(
        ...
        try:
            ...
            # The answer is deliberately not recorded here: this text is
            # pre-output-rail, and ``TextGuardrailsMiddleware.post_invoke``
            # records whatever it actually returns to the caller. That is
            # true whether this function returns a bare string (below, for a
            # string-shaped request) or the ``ChatResponse`` built just below
            # (for a structured ``ChatRequest``): the middleware inspects and
            # protects ``choices[].message.content`` explicitly for the
            # latter, in ``TextGuardrailsMiddleware._post_invoke_chat_response``,
            # rather than relying on NAT's generic top-level-field selector,
            # which would never reach content nested that way.

            # Word counts, not tokens: NAT requires a Usage object and this
            # workflow has no tokeniser for the configured model. Good enough
            # for a rough size signal, never accurate enough to bill on.
            prompt_tokens = sum(
    ...
    async def _stream_fn(
        ...
        try:
            ...
            trace_content.record(run_id, error=f"{type(error).__name__}: {error}")
            raise
        # The answer is deliberately not recorded here: every chunk yielded
        # above is pre-output-rail text. ``TextGuardrailsMiddleware`` sits
        # between this generator and the client, and it records the text it
        # actually releases downstream (see text_guardrails.py). That capture
        # runs whether this generator finishes normally, blocks partway
        # through the output rail, raises, or is cancelled by a client
        # disconnect, so no equivalent ``finally`` is needed here.
