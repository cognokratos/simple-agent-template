...
const MAX_RATIONALE_CHARS: usize = 4000;
const MAX_OPTION_LABEL_CHARS: usize = 120;
const MAX_OPTION_ID_CHARS: usize = 64;
const MAX_OPTION_VALUE_CHARS: usize = 120;
const MAX_OPTION_DESCRIPTION_CHARS: usize = 500;

/// The one choice value this layer understands, because cancelling is part of
/// the interaction protocol rather than part of any application's vocabulary.
...
pub enum InteractionResponsePayload {
    ...
    /// A choice among application-defined options. The value is the choice
    /// itself rather than a bool, so an application can offer more than yes/no.
...
/// Structural validation of a human's answer to an approval prompt.
///
/// This layer deliberately does **not** decide which choices are legitimate.
/// It cannot: the set of valid options belongs to the pending interaction, which
/// lives in the agent that created the prompt, and differs per application and
/// per request. A gateway allow-list would either hardcode one application's
/// vocabulary into shared infrastructure, or — far worse — be dropped entirely
/// and leave the option set under the browser's control.
///
/// So responsibility is split, and both halves are required:
///
/// * here: shape, size and encoding. A body that is too large, an option with
///   an unbounded label, or a field the schema does not declare never reaches
///   the agent.
/// * in the agent (`nat_streaming_react.approval`): the submitted option id is
///   checked against the options that *this* pending interaction actually
///   offered, for the authenticated user who owns that execution.
///
/// Anything this function accepts is still fully re-validated upstream. Passing
...
pub fn validate_interaction_response(
    ...
    match &request.response {
        InteractionResponsePayload::Text { text } => {
            ...
            if trimmed.is_empty() || trimmed.chars().count() > MAX_RATIONALE_CHARS {
                return Err(GatewayError::bad_request(format!(
                    "rationale text must contain 1-{MAX_RATIONALE_CHARS} characters"
                )));
            }
        }
        InteractionResponsePayload::BinaryChoice { selected_option } => {
            // A binary choice is part of the interaction protocol, not of any
            // application's vocabulary, so it *is* checked here: an option
            // labelled "cancel" that reports `true` would read upstream as an
            // approval, which is a confused-deputy shape regardless of domain.
            let consistent = match selected_option.id.as_str() {
            ...
            check_bounds(&selected_option.id, &selected_option.label, None, None)?;
        }
        InteractionResponsePayload::Radio { selected_option } => {
            // Cancelling is protocol-level, so its id and value must agree.
            // Everything else is application-defined and validated upstream.
            let cancelling = selected_option.id == "cancel"
                || selected_option.value == CANCEL_SENTINEL;
            if cancelling
                && !(selected_option.id == "cancel"
                    && selected_option.value == CANCEL_SENTINEL)
            {
                return Err(GatewayError::bad_request(
                    "a cancellation must use both the cancel id and the cancel value",
                ));
            }
            if selected_option.id.trim().is_empty() {
                return Err(GatewayError::bad_request("a choice must have an id"));
            }
            check_bounds(
                &selected_option.id,
                &selected_option.label,
                Some(&selected_option.value),
                Some(&selected_option.description),
            )?;
        }
    }
    Ok(())
...
fn check_bounds(
    ...
    for (field, text, limit) in [
        ("id", Some(id), MAX_OPTION_ID_CHARS),
        ("label", Some(label), MAX_OPTION_LABEL_CHARS),
        ("value", value, MAX_OPTION_VALUE_CHARS),
        ("description", description, MAX_OPTION_DESCRIPTION_CHARS),
    ] {
        if let Some(text) = text
            && text.chars().count() > limit
        {
            return Err(GatewayError::bad_request(format!(
                "approval option {field} must be at most {limit} characters"
            )));
        }
    }
    Ok(())
...
mod tests {
    ...
    /// Application-defined choices pass structurally. Which ids are legitimate
    /// for a given pending interaction is the agent's decision, not this layer's
    /// -- see `validate_interaction_response`.
    #[test]
    ...
    /// Cancelling is protocol-level, so a half-cancel is rejected here: an
    /// option that claims one and not the other could be read upstream as an
    /// approval of something the user meant to abandon.
    #[test]
    ...
    #[test]
    fn a_choice_must_have_an_id_and_bounded_fields() {
        assert!(validate_interaction_response(&radio("", "")).is_err());
        assert!(validate_interaction_response(&radio("   ", "x")).is_err());
    ...
    #[test]
    fn a_rationale_must_be_present_and_bounded() {
        ...
        assert!(validate_interaction_response(&text("")).is_err());
        assert!(
            validate_interaction_response(&text(&"x".repeat(MAX_RATIONALE_CHARS + 1))).is_err()
        );
    }
